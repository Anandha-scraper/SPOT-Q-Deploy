...
const CupolaHolderLogSheetReport = () => {
  ...
  const loadCurrentData = async () => {
    ...
    try {
      const response = await fetch(
        buildApiUrl(`/api/v1/cupola-logs/filter?startDate=${currentDate}&endDate=${currentDate}`),
  ...
  const loadFilteredData = async () => {
    ...
    try {
      const response = await fetch(
        buildApiUrl(`/api/v1/cupola-logs/filter?startDate=${encodeURIComponent(filterStart)}&endDate=${encodeURIComponent(filterEnd)}`),
        { method: 'GET', credentials: 'include', headers: { 'Content-Type': 'application/json' } }
      );
      const data = await response.json();
      if (data?.success) {
